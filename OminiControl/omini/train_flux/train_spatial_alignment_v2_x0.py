...
from .trainer_v2_x0 import OminiModel, get_config, train
from ..pipeline.flux_omini import Condition, convert_to_condition, generate
from ..pipeline.utils import instantiate_from_config
...
class ImageConditionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        description = self.base_dataset[idx]["text"]
...
def test_function(model, save_path, file_name):
    condition_size = model.training_config["dataset"]["params"]["condition_size"]
    target_size = model.training_config["dataset"]["params"]["target_size"]

    position_delta = model.training_config["dataset"]["params"].get("position_delta", [0, 0])
    position_scale = model.training_config["dataset"]["params"].get("position_scale", 1.0)

    adapter = model.adapter_names[2]
    condition_type = model.training_config["condition_type"]
    test_list = []

    if condition_type in ["canny", "coloring", "deblurring", "depth", "seg"]:
        ...
        condition_img = Image.open("assets/skier_pose.jpg").convert("RGB").resize(condition_size, Image.Resampling.NEAREST)
        condition = Condition(condition_img, adapter, position_delta, position_scale)
        test_list.append((condition, "A woman skiing on the snow."))
    elif condition_type == "depth_pred":
        image = Image.open("assets/vase_hq.jpg")
        image = image.resize(condition_size)
    ...
    for i, (condition, prompt) in enumerate(test_list):
        ...
        # print(f"Images after generation: {type(res.images), len(res.images)}")
...
def main():
    ...
    #     "/home/users/asangare/MultiGen-20M/t2i/raw"
    # )["train"]
    ...
    dataset = instantiate_from_config(config["train"]["dataset"])
