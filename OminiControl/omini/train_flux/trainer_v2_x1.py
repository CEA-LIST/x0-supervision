...
from ..pipeline.eval_callback import ControlFidelityEvalCallback, ControlFidelityEvalCallbackForDetection
...
class OminiModel(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Convert velocity prediction to x1-prediction (clean data prediction)
        # Here sigma_t = 1-t and alpha_t = t
        alpha_t = t_.to(pred.dtype)
        sigma_t = 1-t_.to(pred.dtype)
        d_alpha_t = 1
        d_sigma_t = -1
        small_shift = 1e-17
        x1_est = (sigma_t / (d_alpha_t*sigma_t - alpha_t*d_sigma_t + small_shift)) * (pred - d_sigma_t * x_t / (sigma_t + small_shift))

        # Compute loss
        step_loss = torch.nn.functional.mse_loss(x1_est, x_1, reduction="mean")
        ...
        self.log("loss", step_loss.item(), prog_bar=True, logger=True, on_step=True, on_epoch=True)
...
def train(dataset, trainable_model, config, test_function):
    ...
    exp_version = training_config["exp_version"]
    save_path = os.path.join(training_config.get("save_path", "./output"), f"{exp_name}/{exp_version}")
    training_config["save_path"] = save_path
    ...
    if is_main_process:
        ...
        eval_callback_type = config["eval"].get("eval_callback_type", None)
        eval_callback_cls = ControlFidelityEvalCallback
        if eval_callback_type is not None and eval_callback_type == "detection":
            eval_callback_cls = ControlFidelityEvalCallbackForDetection
        
        eval_callback = eval_callback_cls(
            config["eval"],
            batch_frequency=config["eval"]["eval_freq"],
            eval_batch_size=config["eval"]["eval_batch_size"],
            num_eval_images=config["eval"]["num_eval_images"]
        )
    ...
    trainer = L.Trainer(
        ...
        logger=TensorBoardLogger(save_dir=save_path, name=None, version=run_name),
        log_every_n_steps=training_config.get("log_freq", 1000),
