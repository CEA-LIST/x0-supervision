...
def convert_to_condition(
    ...
    if condition_type == "depth":
        ...
        processor = seg_processor or MaskFormerImageProcessor.from_pretrained(model_path)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = seg_model or MaskFormerForInstanceSegmentation.from_pretrained(model_path).to(device).eval()
        inputs = processor(raw_img, return_tensors="pt").to(device)
        output = model(**inputs)
        seg = processor.post_process_semantic_segmentation(output, target_sizes=[512])[0].cpu().numpy().astype(np.uint8) + 1
