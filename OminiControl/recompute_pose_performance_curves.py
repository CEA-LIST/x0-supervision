...
parser.add_argument("--num_eval_images", type=int, default=32)
...
parser.add_argument("--supervision", type=str, choices=["u", "x0", "x1"])
...
if __name__ == "__main__":
    ...
    step = max(len(dataset) // args.num_eval_images, 1)
    dataset = Subset(dataset, list(range(0, len(dataset), step)))
    ...
    max_steps = 40
    scores = {k: dict(Step=[], Value=[]) for k in control_evaluator.metrics}
    ...
    id = 0
    # count = 0
    for train_step in tqdm(range(1, max_steps+1)):
        (save_folder / f"iter_{train_step*1000}_eval_samples").mkdir(exist_ok=True, parents=True)
        ckpt_path = exp_folder / f"ckpt/{train_step*1000*8}/default.safetensors"
        pipe.unload_lora_weights()
        pipe.load_lora_weights(ckpt_path, adapter_name=adapter_name)
        ...
        for sample in tqdm(dataset):
            ...
            image_id = id
            id += 1
            filename = (save_folder / f"iter_{train_step*1000}_eval_samples/generated_{image_id}.jpg").as_posix()
            pil_generated_image.save(filename)
            

        for k in results.keys():
            scores[k]["Step"].append(train_step*1000)
            scores[k]["Value"].append(np.nanmean(results[k]))
    ...
    for k in scores.keys():
        df = pd.DataFrame(scores[k])
        filename = f"omini_pose_{args.supervision}_{k}"
        if args.version != "":
