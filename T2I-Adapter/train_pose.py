...
from ldm.data.dataset_pose import PoseControl
import argparse
...
if __name__ == '__main__':
    ...
    # dataset
    train_dataset = PoseControl(
        root=opt.train_data_root,
        metadata_path=opt.train_metadata_path,
        keypoints_json_path=opt.train_json_path,
        image_size=opt.W,
        p_drop=opt.p_text_drop
    )
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset)
    val_dataset = PoseControl(
        root=opt.val_data_root,
        metadata_path=opt.val_metadata_path,
        keypoints_json_path=opt.val_json_path,
        image_size=opt.W,
        p_drop=.0
    )
            ...
            if rank==0 and (current_iter+1) % 1000 == 0:
                ...
                for i, data in enumerate(val_dataloader):
                    with torch.no_grad():
                        ...
                        # Image.fromarray(data['pose'].squeeze().cpu().numpy().astype(np.uint8)).save(save_dir / f"controls/{i}.png")
                        Image.fromarray(im_pose).save(save_dir / f"controls/{i}.png")
                        with open(save_dir / f"controls/{i}.json", "w") as f:
                            json.dump(data['gt'][0], f, indent=4)
                        ...
                        for id_sample, x_sample in enumerate(x_samples_ddim):
                            ...
                            gtDts.extend(data['gt'])

                ret = evaluator(genImages, gtDts)
                for met in evaluator.metrics:
                    writer.add_scalar(met, np.nanmean(ret[met]), current_iter+1)
