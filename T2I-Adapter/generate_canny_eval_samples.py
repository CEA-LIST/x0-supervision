...
if __name__ == "__main__":
    ...
    for batch in tqdm(dataloader):
        ...
        features_adapter = model_ad(canny.to(device))
        ...
        # print(canny.shape)
        controls = (canny * 255).clamp(0, 255).numpy()[:, 0].astype(np.uint8)
        ...
        for idx in range(bs):
            ...
            # Image.fromarray((canny[idx] * 255).clamp(0, 255).permute(1, 2, 0).numpy().astype(np.uint8)).save(save_folder / f"control_{image_id}.png")
