...
from ldm.data.dataset_canny import CannyControl
import argparse
...
from utils.canny_evaluator import CannyControlEvaluator
...
if __name__ == '__main__':
    ...
    # Evaluator
    evaluator = CannyControlEvaluator()
    ...
    # dataset
    train_dataset = CannyControl(
        metadata_path=opt.train_metadata,
        image_size=opt.W,
        p_drop=opt.p_drop_text
    )
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset)
    val_dataset = CannyControl(
        metadata_path=opt.val_metadata,
        image_size=opt.W,
        p_drop=.0
    )
    num_eval_images = 64
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        for _, data in enumerate(train_dataloader):
            ...
            optimizer.zero_grad()
            model.zero_grad()
            features_adapter = model_ad(canny)
            ...
            if rank==0 and (current_iter+1) % 1000 == 0:
                ...
                for i, data in enumerate(val_dataloader):
                    with torch.no_grad():
                        ...
                        c = model.module.get_learned_conditioning(data['sentence'])
                        canny = data['canny']
                        im_canny = (canny * 255).clamp(0, 255).numpy()[0, 0].astype(np.uint8)
                        # cv2.imwrite(os.path.join(experiments_root, 'visualization', 'mask_it%07d_%04d.png'%(current_iter+1, i)), im_mask)
                        Image.fromarray(im_canny).save(save_dir / f"controls/{i}.png")
                        features_adapter = model_ad(canny)
                        ...
                        for id_sample, x_sample in enumerate(x_samples_ddim):
                            ...
                            gtCannys.append(im_canny)

                ret = evaluator(genImages, np.array(gtCannys))
