...
parser.add_argument(
    ...
    type=float,
    default=0.5,
    help="probability of dropping the text prompt. This is to simultaneously train an unconditional and conditional model. Useful for later classifier-free guidance."
)
parser.add_argument(
    "--bsize",
    type=int,
...
if __name__ == '__main__':
    ...
    # Evaluator
    evaluator = DepthControlEvaluator()
    ...
    train_dataset = DepthDataset(root=data_root, split="train", image_size=opt.W, p_drop=opt.p_text_drop)
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset)
    val_dataset = DepthDataset(root=data_root, split="test", image_size=opt.W, p_drop=.0)
    num_eval_images = 64
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        for _, data in enumerate(train_dataloader):
            ...
            optimizer.zero_grad()
            model.zero_grad()
            features_adapter = model_ad(depth.cuda(non_blocking=True))
            ...
            if rank==0 and (current_iter+1) % 1000 == 0:
                ...
                for i, data in enumerate(val_dataloader):
                    with torch.no_grad():
                        ...
                        c = model.module.get_learned_conditioning(data['sentence'])
                        depth = data['depth']
                        control = (einops.rearrange(depth.squeeze(0), 'c h w -> h w c').cpu()[..., 0] * 255.).numpy().astype(np.uint8)
                        
                        Image.fromarray(control).save(save_dir / f"controls/{i}.png")
                        features_adapter = model_ad(depth.cuda(non_blocking=True))
                        ...
                        for id_sample, x_sample in enumerate(x_samples_ddim):
                            ...
                            gtDepths.append(control)

                ret = evaluator(genImages, np.array(gtDepths))
