...
def DataloaderFromConfig2(batch_size, train=None, validation=None,
                 num_workers=None, shuffle_val_dataloader=False):
    if train is not None:
        train_dataset = instantiate_from_config(train)
        train_dataloader = DataLoader(train_dataset, collate_fn=collate_fn, num_workers=num_workers, batch_size=batch_size, shuffle=True)
    if validation is not None:
        val_dataset = instantiate_from_config(validation)
        val_dataloader = DataLoader(val_dataset, collate_fn=collate_fn, num_workers=num_workers, batch_size=batch_size, shuffle=shuffle_val_dataloader)
