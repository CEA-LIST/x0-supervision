...
class CannyControlEvaluator:
    ...
    def predict_control(self, images):
        ...
        out = []
        for x_ in x:
            out.append(self.canny_detector(x_, 100, 200))
        out = np.stack(out)
        canny = self.postprocess(out)
...
class CannyControlV2Evaluator:
    def __init__(self, eval_dtype=torch.float16):
        self.eval_dtype= eval_dtype
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.canny_detector = CannyDetector()
        self.processor = MaskFormerImageProcessor.from_pretrained("facebook/maskformer-swin-large-ade")
        self.segmentator = MaskFormerForInstanceSegmentation.from_pretrained("facebook/maskformer-swin-large-ade").to(self.device, eval_dtype).eval()
...
class SegmentationControlEvaluator:
    def __init__(self, eval_dtype=torch.float16, ignore_index=0):
        ...
        self.eval_dtype = eval_dtype
        self.processor = MaskFormerImageProcessor.from_pretrained("facebook/maskformer-swin-large-ade")
        self.model = MaskFormerForInstanceSegmentation.from_pretrained("facebook/maskformer-swin-large-ade").to(self.device, eval_dtype).eval()
    ...
    def predict_control(self, images):
        ...
        res = self.postprocess(outputs, len(images), images[0].size[::-1]) + 1 # This is because ids range from [1..N] in ADE20K but [0..N-1] in the MaskFormer
        return res
