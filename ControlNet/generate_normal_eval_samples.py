...
class EvalData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        _, normal  = self.midas(image)
        prompt = row.caption

        normal = normal.astype(np.float32) / 127.5 - 1.
...
if __name__ == "__main__":
    ...
    for batch in tqdm(dataloader):
        ...
        if args.cfg_scale > 1.0:
            ...
            uc = dict(c_concat=[torch.zeros_like(controls)], c_crossattn=[uc_cross])
        ...
        for idx in range(bs):
            ...
            Image.fromarray(generated_image).save(filename)
