...
from utils.control_evaluators import DepthControlEvaluator
...
class EvalData(Dataset):
    def __init__(self, root, image_size):
        super().__init__()
        self.root = root
        self.base_dataset = load_from_disk(root)["test"]
    ...
    def __getitem__(self, idx):
        ...
        pil_depth_image = row["control_depth"].resize((self.image_size, self.image_size), Image.Resampling.NEAREST)
        depth = np.array(pil_depth_image)
        prompt = row["text"]
...
if __name__ == "__main__":
    ...
    dataset = EvalData(args.data_path, args.image_size)
    ...
    for batch in tqdm(dataloader):
        ...
        controls = einops.rearrange(controls, "b c h w -> b h w c").cpu().squeeze(-1).numpy()
        controls = (controls * 255).astype(np.uint8)
